...
1. **Servicios Académicos (API Spring Boot, Puerto 8080)** para consultar el listado de cursos, materias y registrar el completado de recursos.
...
server: {
  proxy: {
    '/api': {
      target: 'http://localhost:8080', // Redirige de forma transparente para evitar errores CORS
      changeOrigin: true,
    }
  }
}
...
* **`vite.config.ts`**:
  * **Contiene:** Configuración del compilador de Vite, plugin oficial de Tailwind CSS v4, plugin de PWA y proxies de reenvío de red (`/api` redirigido a `localhost:8080`).
